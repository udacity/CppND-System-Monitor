...
string Process::getProcess() {
  this->mem = Process::getVmsSize(this->pid);
  this->up_time = Process::getProcUpTime(this->pid);
  this->cpu = Process::getCpuPercent(this->pid);
...
string Process::getProcUser(string pid) 
{
  string line;
  string name = "Uid:";
  string result = "";
...
int Process::getNumberOfRunningProcesses() {
  string line;
  int result = 0;
...
int Process::getTotalNumberOfProcesses() 
{
  string line;
  int result = 0;
...
vector<string> Process::getPidList() {
  DIR* dir;
  ...
  if (!(dir = opendir("/proc"))) throw std::runtime_error(std::strerror(errno));
...
int Process::getTotalThreads() {
  string line;
  int result = 0;
  string name = "Threads:";
  vector<string> _list = Process::getPidList();
...
string Process::getProcUpTime(string pid) {
  string line;
  string value;
  // float result;
  ifstream stream =
...
string Process::getCpuPercent(string pid) {
  string line;
  string value;
  float result;
  ifstream stream =
  ...
  // selected process
  float utime = stof(Process::getProcUpTime(pid));
  float stime = stof(values[14]);
  float cutime = stof(values[15]);
  float cstime = stof(values[16]);
  float starttime = stof(values[21]);
  float uptime = Process::getSysUpTime();
...
string Process::getVmsSize(string pid) {
  string line;
...
string Process::getCmd(string pid) {
  string line;
