...
string ProcessParser::getCpuPercent(string pid) {
  ...
  float result;
  ifstream stream =
      getStream((Path::basePath() + pid + "/" + Path::statPath()));
  getline(stream, line);
  string str = line;
...
long int ProcessParser::getSysUpTime() {
  string line;
  ifstream stream = getStream((Path::basePath() + Path::upTimePath()));
  getline(stream, line);
  istringstream buf(line);
  istream_iterator<string> beg(buf), end;
  vector<string> values(beg, end);
...
string ProcessParser::getCmd(string pid) {
  string line;
  ifstream stream = getStream((Path::basePath() + pid + Path::cmdPath()));
...
int ProcessParser::getNumberOfCores() {
  ...
  ifstream stream = getStream((Path::basePath() + "cpuinfo"));
  while (std::getline(stream, line)) {
    if (line.compare(0, name.size(), name) == 0) {
      istringstream buf(line);
      istream_iterator<string> beg(buf), end;
...
vector<string> ProcessParser::getSysCpuPercent(string coreNumber) {
  ...
  ifstream stream = getStream((Path::basePath() + Path::statPath()));
  while (std::getline(stream, line)) {
    if (line.compare(0, name.size(), name) == 0) {
      istringstream buf(line);
      istream_iterator<string> beg(buf), end;
...
string ProcessParser::getSysKernelVersion() {
  ...
  ifstream stream = getStream((Path::basePath() + Path::versionPath()));
  while (std::getline(stream, line)) {
    if (line.compare(0, name.size(), name) == 0) {
      istringstream buf(line);
      istream_iterator<string> beg(buf), end;
...
string ProcessParser::getOsName() {
  ...
  ifstream stream = getStream(("/etc/os-release"));
