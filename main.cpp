...
void writeSysInfoToConsole(SysInfo sys, WINDOW* sys_win){
    ...
    mvwprintw(sys_win,2,xMax/2,getCString(( "Kernel version: " + sys.getKernelVersion())));
    mvwprintw(sys_win,3,2,getCString(( "Total Processes:" + sys.getTotalProc())));
    mvwprintw(sys_win,3,xMax/2,getCString(( "Running Processes:" + sys.getRunningProc())));
    mvwprintw(sys_win,4,2,getCString(( "Up Time: " + Util::convertToTime(sys.getUpTime()))));
    mvwprintw(sys_win,4,xMax/2,getCString(( "Memory: ")));
...
void getProcessListToConsole(std::vector<string> processes,WINDOW* win){
    ...
    mvwprintw(win,1,2,"  PID: |   User:  |CPU%%|  RAM  | Uptime| CMD");
